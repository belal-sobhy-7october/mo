...
app.post("/api/sessions/:sessionId/mobility", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM mobility_tests WHERE session_id = $1", [
      sessionId,
    ]);
...
app.post("/api/sessions/:sessionId/stability", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM stability_tests WHERE session_id = $1", [
      sessionId,
    ]);
...
app.post("/api/sessions/:sessionId/strength", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM strength_tests WHERE session_id = $1", [
      sessionId,
    ]);
...
app.post("/api/sessions/:sessionId/benchmarks", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM strength_benchmarks WHERE session_id = $1", [
      sessionId,
    ]);
